...
@app.get("/contacto", response_model=list[Contacto])
async def get_contacto():
    # Lee el contenido del archivo CSV
    try:
        with open("contacto.csv", newline="", encoding="utf-8") as csvfile:
            reader = csv.DictReader(csvfile)
            contactos_lista = [{"nombre": row["nombre"], "pimerApellido": row["primerApellido"], "segundoApellido": row["segundoApellido"], "email": row["email"], "telefono": row["telefono"]} for row in reader]
...
@app.post("/contacto", response_model=Contacto)
async def add_contacto(contacto: Contacto):
    # Agrega el nuevo contacto a la lista
    contactos_lista.append({"nombre": contacto.nombre,"primerApellido": contacto.pimerApellido, "segundoApellido": contacto.segundoApellido, "email": contacto.email, "telefono": contacto.telefono})
    ...
    try:
        with open("contactos.csv", mode="a", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow({"nombre": contacto.nombre,"primerApellido": contacto.pimerApellido, "segundoApellido": contacto.segundoApellido, "email": contacto.email, "telefono": contacto.telefono})
